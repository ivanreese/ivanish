  var running, F2, F3, G2, G3, Grad, PI, TAU, clip, fade, grad3, gradP, lerp, p, perlin2, perlin3, perm, scale, seed, simplex2, simplex3;
...
var container = document.getElementById("wiggle");

var API, absolutePos, blurSamples, blurTime, count, dpr, height, l, len, makeSurface, name, noiseRadius, phase, ref, render, renderMain, renderRequested, requestRender, requestResize, resize, surfaceNames, surfaces, width, x1, x2, y1, y2;
...
x1 = function(t) {
  seed = 317;
  return 0.25 * width + 100 * simplex2(seed + noiseRadius * Math.cos(TAU * t / 20), noiseRadius * Math.sin(TAU * t / 20));
};
y1 = function(t) {
  seed = 1697;
...
x2 = function(t) {
  seed = 1317;
  return 0.75 * width + 100 * simplex2(seed + noiseRadius * Math.cos(TAU * t / 40), noiseRadius * Math.sin(TAU * t / 40));
};
y2 = function(t) {
  seed = 697;
...
renderMain = function(time, blur) {
  ...
  for (i = l = 0, ref = blurSamples; 0 <= ref ? l <= ref : l >= ref; i = 0 <= ref ? ++l : --l) {
    ...
    ctx.strokeWidth = 2;
    ctx.fillStyle = "hsl(311 100% 10% / 1)";
    ctx.strokeStyle = "hsl(0 100% 100% / .5)";
    ctx.beginPath();
    ...
    ctx.beginPath();
    ctx.strokeWidth = 2;
    ctx.strokeStyle = "hsl(311 100% 10% / .2)";
    ctx.moveTo(x1(t), y1(t));
    steps = 100;
    delay = (Math.sin(TAU * time / 20)/2 + .5) * 15 + 5;
